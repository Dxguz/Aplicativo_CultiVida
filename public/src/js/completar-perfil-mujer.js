...
function toggleRequiredByRole(role) {
  const profesionalRequired = [tipoDocumento, institucion, expCantidad];

  profesionalRequired.forEach(el => {
    if (!el) return;
    el.required = role === 'profesional';
    // NO deshabilitar inputs
    el.disabled = false;
  });

  if (docFile) docFile.disabled = false;
}
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (!snap.exists()) {
    ...
    currentRole = 'mujer_rural';
    roleSelect.value = 'mujer_rural';
...
profileForm.addEventListener('submit', async (e) => {
  ...
  if (
    ...
    !telefono.value.trim() ||
    !paisSelect.value ||
    (paisSelect.value === "Colombia" && !municipioSelect.value)
  ) {
    showMsg('Todos los campos son obligatorios (excepto descripción)');
    return;
  }
  ...
  if (!regexSoloLetras.test(firstName.value.trim())) {
    showMsg('El nombre solo puede contener letras');
    return;
  }

  if (!regexSoloLetras.test(lastName.value.trim())) {
  ...
  try {
    ...
    // =====================
    // Obtener datos actuales SIEMPRE
    ...
    if (fotoFile.files[0]) {
      ...
      // Mantener SIEMPRE la que ya existe en DB
      data.photoURL = dataActual.photoURL || user.photoURL || null;
    }
...
cancelBtn.addEventListener('click', () => {
  ...
  // Si es primera vez (no ha completado perfil)
  if (!profileCompleted) {
    window.location.href = 'index.html';
    return;
  }

  // Si ya tiene perfil completo → volver a su panel según rol
  if (currentRole === 'profesional') {
  ...
  else if (currentRole === 'mujer_rural') {
